...
GaryScript2:
	...
	ld a, [wRivalStarter + 1]					; to handle 2 bytes species IDs
	ld c, a										; to handle 2 bytes species IDs
	ld hl, STARTER2								; to handle 2 bytes species IDs
	call CompareBCtoHL							; to handle 2 bytes species IDs
	jr nz, .NotStarter2
	ld a, $1
	jr .saveTrainerId
.NotStarter2
	ld hl, STARTER3								; to handle 2 bytes species IDs
	call CompareBCtoHL							; to handle 2 bytes species IDs
	jr nz, .NotStarter3
	ld a, $2
	jr .saveTrainerId
...
.saveTrainerId
	ld b, a
	call CheckE4RematchConditions
	jr c, .next									; carry flag means conditions are not met for using rematch teams
	inc b										; if conditions are met, increase the trainer number by 3 to use rematch teams
	inc b
	inc b
.next
	ld a, b
...
GaryText3:
	...
	ld a, [wPlayerStarter + 1]			; to handle 2 bytes species IDs
