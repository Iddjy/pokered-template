...
PokemonTower6Script0:
	...
	call DisplayTextID
	ld bc, GHOST_MAROWAK				; to handle 2 bytes species IDs
	ld a, b								; to handle 2 bytes species IDs
	ld [wCurOpponent], a				; to handle 2 bytes species IDs
	ld a, c								; to handle 2 bytes species IDs
	ld [wCurOpponent + 1], a			; to handle 2 bytes species IDs
	ld a, 30
	ld [wCurEnemyLVL], a
	xor a
	ld [wEngagedTrainerClass], a		; need to clear this in case it was clobbered by an in-game trade NPC before initiating the battle
	ld a, $4
	ld [wPokemonTower6FCurScript], a
	ld [wCurMapScript], a
	ret
...
PokemonTower6Text7:
	...
	call PrintText
	ld bc, MAROWAK							; to handle 2 bytes species IDs
	call PlayCry
