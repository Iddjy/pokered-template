...
.asm_1cd32
	...
	ld a, [wRivalStarterTemp + 1]		; to handle 2 bytes species IDs
	ld [wRivalStarter + 1], a			; to handle 2 bytes species IDs
...
OaksLabScript11:
	...
	ld a, SONY1
	ld [wEngagedTrainerClass], a	; also need this since now we use it to distinguish wild from trainer
	ld [wCurOpponent + 1], a
	ld a, [wRivalStarter]
	ld b, a
...
OaksLabText2:
	TX_ASM
	ld bc, STARTER2							; to handle 2 bytes species IDs
	ld a, b									; to handle 2 bytes species IDs
	ld [wRivalStarterTemp], a				; to handle 2 bytes species IDs
	ld a, c									; to handle 2 bytes species IDs
	ld [wRivalStarterTemp + 1], a			; to handle 2 bytes species IDs
	ld a, $3
	ld [wRivalStarterBallSpriteIndex], a
	ld de, STARTER1							; to handle 2 bytes species IDs
	ld b, $2
	jr OaksLabScript_1d133

OaksLabText3:
	TX_ASM
	ld bc, STARTER3							; to handle 2 bytes species IDs
	ld a, b									; to handle 2 bytes species IDs
	ld [wRivalStarterTemp], a				; to handle 2 bytes species IDs
	ld a, c									; to handle 2 bytes species IDs
	ld [wRivalStarterTemp + 1], a			; to handle 2 bytes species IDs
	ld a, $4
	ld [wRivalStarterBallSpriteIndex], a
	ld de, STARTER2							; to handle 2 bytes species IDs
	ld b, $3
	jr OaksLabScript_1d133

OaksLabText4:
	TX_ASM
	ld bc, STARTER1							; to handle 2 bytes species IDs
	ld a, b									; to handle 2 bytes species IDs
	ld [wRivalStarterTemp], a				; to handle 2 bytes species IDs
	ld a, c									; to handle 2 bytes species IDs
	ld [wRivalStarterTemp + 1], a			; to handle 2 bytes species IDs
	ld a, $2
	ld [wRivalStarterBallSpriteIndex], a
	ld de, STARTER3							; to handle 2 bytes species IDs
	ld b, $4
...
OaksLabScript_1d157:
	...
	ld hl, wd730
	set 6, [hl]
	call GetDexNumberBySpeciesID			; to handle species with multiple forms
	predef StarterDex
	call GetMonName							; to allow any pokemon to be used as starter (to replace hardcoded starter names in dialog)
	call GetMonHeader						; to get the mon's type
	ld a, [wMonHType1]						; use Type 1 in the dialog
	ld d, a									; input for next function
	callab FarGetTypeName					; puts the type name in cf4b
	ld hl, wcf4b
	call SetStringToLowerCase
	ld hl, wd730
	res 6, [hl]
	call ReloadMapData
	ld c, 10
	...
	ld [wPlayerStarter], a				; to handle 2 bytes species IDs
	ld a, [wMonSpeciesTemp + 1]			; to handle 2 bytes species IDs
	ld c, a								; to handle 2 bytes species IDs
	ld [wPlayerStarter + 1], a			; to handle 2 bytes species IDs
	call GetMonName
	ld a, [wSpriteIndex]
	cp $2
	jr nz, .asm_1d1db
...
OaksLabText5:
	...
	jr z, .asm_1d279
.asm_1d266
	CheckEvent EVENT_GOT_BATTLE_PASS
	jr nz, .usualText
	ld a, [wNumHoFTeams]
	and a
	jr z, .usualText
	ld hl, OaksLabBattlePassText
	call PrintText
	lb bc, BATTLE_PASS, 1
	call GiveItem
	ld hl, OakLabBattleFacilityText
	call PrintText
	SetEvent EVENT_GOT_BATTLE_PASS
	jp TextScriptEnd
.usualText
...
.asm_1d2b8
	...
	xor a
	ld [wBagSavedMenuItem], a				; add this to avoid the cursor ending up below CANCEL in the items menu
	ld a, $f
	ld [wOaksLabCurScript], a
	jr .asm_1d2ed
.asm_1d2c8
...
OaksLabBattlePassText:
	TX_FAR _OaksLabBattlePassText
	TX_SFX_KEY_ITEM
	db "@"

OakLabBattleFacilityText:
	TX_FAR _OakLabBattleFacilityText
	db "@"
