...
BIT_CRY_SFX       EQU 1 ; if the sound playing is a cry

; replace the sparse ids with a normal list of sequential ids
...
const_value = 1
	...
	const SFX_SNARE_4
	const SFX_SNARE_5
	const SFX_TRIANGLE_1
	const SFX_TRIANGLE_2
	const SFX_SNARE_6
	const SFX_SNARE_7
	const SFX_SNARE_8
	const SFX_SNARE_9
	...
	const SFX_CYMBAL_2
	const SFX_CYMBAL_3
	const SFX_MUTED_SNARE_1
	const SFX_TRIANGLE_3
	const SFX_MUTED_SNARE_2
	const SFX_MUTED_SNARE_3
	const SFX_MUTED_SNARE_4

MAX_INSTRUMENT_ID EQU const_value + -1
	...
	const SFX_GO_INSIDE
	const SFX_SWAP
	const SFX_POKEFLUTE_SHORT			; create a constant for the short version of the flute used in battle
	const SFX_PURCHASE
	const SFX_COLLISION
...
BATTLE_SFX_START EQU const_value		; add this to use instead of magic number
...
; use a separate set of constants for base cries, so that we can add more
const_value = 1
