...
NUM_OUTSIDE_MAPS EQU const_value		; all maps whose IDs are after this one are considered "interior" maps, all those before are "outside" maps (more like cities/routes)
										; it is technically equal to the next map ID, but this constant's name is more explicit and having a dedicated one allows changing the next map ID without having to change it also in the code that checks if a map is "outside"
	...
	mapconst ROUTE_23_HYPER_TRAINING_ROOM,   4,  5 ; $6D
	...
	mapconst BATTLE_FACILITY,                4,  6 ; $F8
	mapconst BATTLE_FACILITY_ELEVATOR,       2,  2 ; $F9
