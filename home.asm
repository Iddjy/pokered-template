...
; add this function to read sound data from bank a at offset de into wCurMusicByte
LoadMusicByte::
	push af
	ld a, [H_LOADEDROMBANK]
...
; input: species ID in bc
; output: dex number in bc
; clobbbers wMonSpeciesTemp and wMonDexNumber
ConvertSpeciesIDtoDexNumber::
...
; play cry from species ID
PlayCry::
	call ConvertSpeciesIDtoDexNumber
PlayCryFromDexNumber::						; this will be (directly) used only from the pokedex menu
; Play monster bc's cry.
	call GetCryData
	call PlayCrySound						; use new entrypoint for cries
	call WaitForSoundToFinish
	ld hl, wFrequencyModifier
	xor a									; reset modifiers
	ld [hli], a
	ld [hli], a
	ld [hli], a
	ld [hl], a
...
GetCryData::
; Load cry data for monster bc.	; use bc as input instead of a since species ID are now 2 bytes long
	dec bc						; to handle 2 bytes species ID
	ld hl, CryData
	add hl, bc
	add hl, bc
	add hl, bc
	add hl, bc					; to handle 2-bytes modifiers
	add hl, bc					; to handle 2-bytes modifiers
	add hl, bc					; to handle 2-bytes base cry IDs

	ld a, BANK(CryData)
	call BankswitchHome
	ld a, [hli]
	ld b, a ; cry id first byte
	ld a, [hli]
	ld c, a	; cry id second byte
	ld a, [hli]
	ld [wFrequencyModifier], a
	ld a, [hli]							; use 2 bytes for frequency modifier
	ld [wFrequencyModifier + 1], a		; use 2 bytes for frequency modifier
	ld a, [hli]							; use 2 bytes for tempo modifier
	ld [wTempoModifier], a				; use 2 bytes for tempo modifier
	ld a, [hl]							; use 2 bytes for tempo modifier
	ld [wTempoModifier + 1], a			; use 2 bytes for tempo modifier
	call BankswitchBack
...
HandlePartyMenuInput::
	...
	ld [wBattleMonSpecies2], a			; to handle 2 bytes species IDs
...
; function to get the catch rate of a pokemon from its ID and set it in wEnemyMonActualCatchRate
; input: [wMonSpeciesTemp]/[wMonSpeciesTemp + 1] = pokemon species ID
GetMonCatchRate::
	jpab GetMonCatchRate_

; map a species ID in [wMonSpeciesTemp]/[wMonSpeciesTemp + 1] to a dex number in [wMonDexNumber]/[wMonDexNumber + 1]
GetDexNumberBySpeciesID::
	ld a, [H_LOADEDROMBANK]
	push af
	ld a, BANK(BaseStats)
	ld [H_LOADEDROMBANK], a
	...
	ld hl, BaseStats
	call AddNTimes16Bits
	ld de, wMonDexNumber
	ld bc, $0002
	call CopyData
	ld hl, wMonDexNumber			; need to reverse wMonDexNumber and wMonDexNumber + 1
	ld a, [hli]
	ld b, a
	ld a, [hld]
	ld [hli], a
...
; INPUT:
; [wMonSpeciesTemp]/[wMonSpeciesTemp + 1] = pokemon ID
GetMonHeader::
	ld a, [H_LOADEDROMBANK]
	push af
	ld a, BANK(BaseStats)
	...
	ld hl, FOSSIL_KABUTOPS			; to handle 2 bytes species IDs
	call CompareBCtoHL				; to handle 2 bytes species IDs
	ld de, FossilKabutopsPic
	ld a, $66						; size of Kabutops fossil sprite
	jr z, .specialID
	ld hl, MON_GHOST				; to handle 2 bytes species IDs
	call CompareBCtoHL				; to handle 2 bytes species IDs
	ld de, GhostPic
	ld a, $66						; size of Ghost sprite
	jr z, .specialID
	ld hl, FOSSIL_AERODACTYL		; to handle 2 bytes species IDs
	call CompareBCtoHL				; to handle 2 bytes species IDs
	ld de, FossilAerodactylPic
	ld a, $77						; size of Aerodactyl fossil sprite
	jr z, .specialID
	ld d, b							; to handle 2 bytes species IDs
	ld e, c							; to handle 2 bytes species IDs
...
.specialID
	...
	ld [hl], a ; write sprite dimensions
	inc hl
	ld [hl], e ; write front sprite pointer
	inc hl
	ld [hl], d
	ld hl, NUM_POKEMON + 1			; set a special, otherwise unused dex number for special IDs
	ld a, h
	ld [wMonHIndex], a
	ld a, l
	ld [wMonHIndex + 1], a
	jr .end
.done
	ld hl, wMonHIndex				; to handle 2 bytes species IDs (need to reverse wMonHIndex and wMonHIndex + 1)
	ld a, [hli]						; to handle 2 bytes species IDs
	ld b, a							; to handle 2 bytes species IDs
	ld a, [hld]						; to handle 2 bytes species IDs
	ld [hli], a						; to handle 2 bytes species IDs
	ld [hl], b						; to handle 2 bytes species IDs
.end
	pop hl
	pop de
	pop bc
...
; output: if input is an upper-case letter, output is that letter in lower-case, otherwise output = input
ToLowerCase:
	cp "A"
	ret c
	cp "Z" + 1
...
; sets string in hl to LowerCase
SetStringToLowerCase:
	ld a, [hl]
	call ToLowerCase
	ld [hli], a
	cp "@"
	jr nz, SetStringToLowerCase
	ret
...
; sets carry flag is bc < hl
; unsets carry flag if bc >= hl
; sets z flag is bc = hl
CompareBCtoHL::
	ld a, b
	sub h
	ret nz					; if high bytes are different, no need to check low byte, carry flag will tell which one is bigger
	ld a, c
	sub l
	ret
...
UncompressMonSprite::
	...
	callab GetMonPicsBank		; gets the mon's pics bank in d
	ld a, d						; previous function stores the bank in d, so load it in a for next function
...
DisplayPCBoxFullText::
	callab PrintPCBoxFullText
	jp AfterDisplayingTextID
...
CountSetBits_16bits_:
	ld a, [H_LOADEDROMBANK]
	push af
	ld a, BANK(CountSetBits_16bits)
	ld [H_LOADEDROMBANK], a
	ld [MBC1RomBank], a
	call CountSetBits_16bits
	pop af
	ld [H_LOADEDROMBANK], a
	ld [MBC1RomBank], a
	ret
...
.bankswitch
	...
	ld hl, wd730
	set 6, [hl]							; turn off letter printing delay
DisplayListMenuIDMain::					; added this entry point to handle page switching
	ld a, [wBattleType]
	and a								; is it the Old Man battle?
	jr nz, .skipUpdate					; skip updating the list in that case because it would clobber the Old Man bag item list
	call UpdateListAddress				; update list according to current page in case of the items menu
.skipUpdate
	xor a
...
.buttonAPressed
	...
	ld [wWhichPokemon], a
	ld a, [wListMenuID]
	and a							; to handle 2 bytes species IDs
	jr z, .doubleItemIndex			; to handle 2 bytes species IDs
	cp ITEMLISTMENU
	jr c, .skipMultiplying
.doubleItemIndex					; if it's an item menu or pc pokemon menu
	sla c							; item entries are 2 bytes long, so multiply by 2
.skipMultiplying
	ld a, [wListPointer]			; pointer to the whole list of items
	ld l, a
	ld a, [wListPointer + 1]
	ld h, a
	inc hl							; hl = beginning of list entries
	ld b, 0
	add hl, bc						; add the offset calculated above to get to the address of the selected item ID
	ld a, [hl]
	ld [wcf91], a					; store selected item ID in wcf91
	ld a, [wListMenuID]
...
.skipGettingQuantity
	ld a, [wcf91]
	ld [wd11e], a					; replaced wd0b5 with wd11e since we changed from GetName to GetItemName
	ld a, BANK(ItemNames)
	ld [wPredefBank], a
	call GetItemName				; changed from GetName because when we are in the TMs page, we get the move names
									; for each TM, which overwrites wNameListType with MOVE_NAME, and we want ITEM_NAME here
	jr .storeChosenEntry
.pokemonList
	ld a, [hli]						; to handle 2 bytes species IDs
	ld [wMonSpeciesTemp], a			; to handle 2 bytes species IDs
...
.checkOtherKeys						; check B, SELECT, Up, and Down keys
	...
	bit BIT_SELECT, a				; was the select button pressed?
	jp nz, HandleItemListSwapping	; if so, allow the player to swap menu entries
	ld b, a
	bit BIT_D_DOWN, b				; was Down pressed?
...
GoToNextPage::
	...
	cp ITEMLISTMENU
	ret nz							; if we're not in the items menu, don't use pages
.do
	call GoToNextPage_
	jp DisplayListMenuIDMain
...
PrintListMenuEntries::
	...
	ld a, "─"					; add this to overwrite the blinking arrow that's now in the menu frame
	Coorda 18, 12				; add this to overwrite the blinking arrow that's now in the menu frame
	ld a, [wListPointer]
	ld e, a
	ld a, [wListPointer + 1]
	ld d, a
	...
	jr z, .doubleItemIndex		; to handle 2 bytes species IDs
	ld a, [wListMenuID]			; to handle 2 bytes species IDs
	cp ITEMLISTMENU
	ld a, c
	jr c, .skipMultiplying
.doubleItemIndex				; if it's an item menu or a pc pokemon menu
...
.printItemPrice
	...
	add hl, bc
	ld a, [wPrintItemPrices]			; to handle multiple currencies
	dec a								; to handle multiple currencies
	jr z, .printMoneyPrice				; to handle multiple currencies
	ld c, 3								; to handle multiple currencies
	ld b, 1								; to handle multiple currencies
	call PrintNumber					; to handle multiple currencies
	jr .skipPrintingItemPrice			; to handle multiple currencies
.printMoneyPrice						; to handle multiple currencies
...
.skipPrintingPokemonLevel
	pop hl
	pop de
	inc de								; move to next item in menu
	ld a, [wListMenuID]
	cp ITEMLISTMENU_NOQUANTITY			; changed from ITEMLISTMENU to enable swapping in Key items page
	jr c, .nextListEntry
	jr nz, .printItemQuantity
	ld a, [wItemMenuCurrentPage]		; if we're here this means we're either on the TMs page or on the Key items page
	cp TM_PAGE_NUMBER					; check if we're on the TMs page
	jr nz, .notTMPage					; if we're not, don't display the TM move
	call DisplayTMMove					; if we're on the TMs page, display the move name under the TM name
.notTMPage
	pop bc
	push bc
	inc c								; necessary for wMenuItemToSwap to match item index from 1 byte pockets
	sla c								; necessary to match the sla a applied to wMenuItemToSwap later
	jr .checkSwapping
.printItemQuantity
	push hl
...
.nextListEntry
	...
	jr nz, .notPCPokemonMenu	; to handle 2 bytes species IDs
	inc de
.notPCPokemonMenu
	...
	dec b
	jp nz, .loop
	ld bc, 12				; 12 instead of -8 to move the arrow into the menu frame
	add hl, bc
	ld a, "▼"
	ld [hl], a
	ret
.printCancelMenuItem
	ld a, [wListMenuID]			; to handle 2 bytes species IDs
	and a
	jr nz, .printCancel			; if it's not a pc pokemon menu, no need to check for double $ff terminator
	inc de
	ld a, [de]					; check the next byte to confirm if this is really the terminator
	dec de						; rewind de once in case this is not the terminator but just a species ID with $ff
	cp $ff
	jp nz, .notCancel			; if next byte is not $ff, jump back into the loop 
.printCancel					; 
...
; get name from species ID
GetMonName::
	call GetDexNumberBySpeciesID
GetMonNameByDexNumber::
	push hl
	ld a, [H_LOADEDROMBANK]
	push af
...
GetDefenderMoveName::
	ld a, [H_WHOSETURN]
	xor 1
	jr GetAttackerMoveName.test

GetAttackerMoveName::
	ld a, [H_WHOSETURN]
.test
	and a
	jr nz, GetEnemyMoveName
	; fallthrough
...
; input: species id in wMonSpeciesTemp/wMonSpeciesTemp+1 in case of signature moves
; output: move name stored starting at wcd6d
GetMoveName::
	push hl
	push bc							; since we bankswitch in this function, save bc before
	ld a, [wd11e]
	ld [wd0b5], a
	ld hl, wNewBattleFlags
	bit USING_SIGNATURE_MOVE, [hl]	; if it's a signature move whose id was already mapped
	jr nz, .signatureMoveMapped
	cp SIGNATURE_MOVE_1
	jr z, .signatureMove
	cp SIGNATURE_MOVE_2
...
.signatureMoveMapped
	ld [wd0b5], a
	ld a, SIGNATURE_MOVE_NAME
	ld [wNameListType], a
	ld a, BANK(SignatureMoveNames)
	jr .gotListTypeAndBank
.notSignatureMove
	ld a, MOVE_NAME
	ld [wNameListType], a
	ld a, BANK(MoveNames)
...
;    a -> victory text/static mon ID (into hl)
...
.trainerNotYetFought
	...
	call PrintText
	ld a, $a
	call ReadTrainerHeaderInfo		; repurposed the unused victory text pointer to hold static wild mons IDs
	ld a, h							; store static mon ID in wMonSpeciesTemp/wMonSpeciesTemp+1 for battle init later
	ld [wMonSpeciesTemp], a
	ld a, l
	ld [wMonSpeciesTemp+1], a
	ld a, $8
...
EndTrainerBattle::
	...
	cp STATIC_MON
	jr nz, .skipRemoveSprite    ; if it's not a static mon, don't remove its sprite
...
InitBattleEnemyParameters::
	...
	cp STATIC_MON						; test if wEngagedTrainerClass is STATIC_MON to determine whether this is a trainer or a static mon
	ld a, [wEngagedTrainerSet]
	jr z, .noTrainer					; if wEngagedTrainerClass is STATIC_MON, jump
	ld [wTrainerNo], a
	ret
.noTrainer
	ld [wCurEnemyLVL], a
	ld a, [wMonSpeciesTemp]				; get the static mon ID from wMonSpeciesTemp and overwrite wCurOpponent with it
	ld [wCurOpponent], a
	ld a, [wMonSpeciesTemp+1]			; to handle 2 bytes species IDs
	ld [wCurOpponent+1], a
	ret
...
GetName::
; arguments:
; [wd0b5] = which name for names other than monster names
; [wMonSpeciesTemp]/[wMonSpeciesTemp + 1] = species ID for monster names
...
.otherEntries
	...
	; BUG: This applies to all names except monster names instead of just items.
	cp ITEM_NAME				; fix bug described above
	jr nz, .notTMorHM
	ld a, [wd0b5]				; moved from the beginning of the function
	ld [wd11e], a				; moved from the beginning of the function
	cp HM_01					; moved from the beginning of the function
	jr c, .notTMorHM			; moved from the beginning of the function
	call GetMachineName			; moved from the beginning of the function
	jr .gotPtr					; need to drain the stack before returning
.notTMorHM
	ld a, [wPredefBank]
	ld [H_LOADEDROMBANK], a
	ld [MBC1RomBank], a
...
.skipAnimation
	coord hl, 18, 17				; display the blinking arrow in the textbox frame
...
; calculates all stats of current mon and writes them to [de]
; expects wCurEnemyLVL to contain the mon's level and wMonHeader to be populated with the Mon's data (through GetMonHeader)
CalcStats::
	ld c, $0
.statsLoop
	inc c
...
; c: stat to calc (HP=1, Attack=2, Defense=3, Speed=4, Special Attack=5, Special Defense=6)
; b: consider stat exp?
; hl: pointer to the byte just before the mon's EVs (eg: wPartyMon1EVs - 1)
; expects wCurEnemyLVL to contain the mon's level and wMonHeader to be populated with the Mon's data (through GetMonHeader)
CalcStat::
	push hl
	push de
	...
	ld d, a
	push hl
	ld hl, wMonHBaseStats - 1	; more logical than starting from wMonHeader, and avoids the problem of the species ID's size (2 bytes now)
	ld b, $0
	add hl, bc
	...
	ld a, d
	and a
	jr z, .EVsDone		; consider EVs?
	add hl, bc          ; skip to corresponding EV value
	ld b, [hl]
.EVsDone
	pop hl
	push bc
	ld bc, wPartyMon1DVs - (wPartyMon1HPEV - 1)	; has to be the same length as wEnemyMonDVs - wEnemyMonHP
	add hl, bc
	pop bc
	ld a, c
	cp ATTACK_STAT
	...
	cp SPEED_STAT
	jr z, .getSpeedIV
	cp SPECIAL_ATTACK_STAT
	jr z, .getSpecialAttackIV
	cp SPECIAL_DEFENSE_STAT			; for special defense 
	jr z, .getSpecialDefenseIV		; for special defense
.getHpIV
	inc hl							; use new dedicated DV for HP (stored in the same byte as the speed DV)
	ld a, [hl]						; use new dedicated DV for HP
	and $f							; use new dedicated DV for HP
	jr .calcStatFromIV
.getAttackIV
	ld a, [hl]
	swap a
...
.getSpecialAttackIV
	inc hl
	inc hl					; now special attack DV is stored one byte higher with special defense instead of with speed
	ld a, [hl]
	and $f
	jr .calcStatFromIV		; add this for new DV for special defense
.getSpecialDefenseIV		; add this for new DV for special defense
	inc hl					; add this for new DV for special defense
	inc hl					; add this for new DV for special defense
	ld a, [hl]				; add this for new DV for special defense
	swap a					; add this for new DV for special defense
	and $f					; add this for new DV for special defense
.calcStatFromIV
	ld d, $0
	add e
	ld e, a
...
.noCarry2
	...
	ld a, [wCurEnemyLVL]
	ld [H_MULTIPLIER], a
	call Multiply            ; ((Base + IV) * 2 + EV / 4) * Level
	ld a, [H_MULTIPLICAND]
	ld [H_DIVIDEND], a
	ld a, [H_MULTIPLICAND+1]
	ld [H_DIVIDEND+1], a
	...
	ld a, $3
	ld b, a
	call Divide             ; (((Base + IV) * 2 + EV / 4) * Level) / 100
	ld a, c
	cp $1
	...
	ld a, [H_MULTIPLICAND+1]
	inc a
	ld [H_MULTIPLICAND+1], a ; HP: (((Base + IV) * 2 + EV / 4) * Level) / 100 + Level
.noCarry3
...
.notHPStat
	...
	jr nc, .noCarry4
	ld a, [H_MULTIPLICAND+1]
	inc a                    ; non-HP: (((Base + IV) * 2 + EV / 4) * Level) / 100 + 5
	ld [H_MULTIPLICAND+1], a ; HP: (((Base + IV) * 2 + EV / 4) * Level) / 100 + Level + 10
.noCarry4
...
.getJoypadState
	...
	jr nz, .keyPressed
	push hl
	coord hl, 18, 12 				; change this from 18, 11 to make the arrow appear in the menu frame
									; coordinates of blinking down arrow in some menus (eg Bag)
...
.checkOtherKeys
	...
	ld a, [hJoy5]
	bit BIT_D_RIGHT, a
	jr z, .checkLeftButtonPressed
	call GoToNextPage
	jr .checkIfAButtonOrBButtonPressed
.checkLeftButtonPressed
	bit BIT_D_LEFT, a
	call nz, GoToPreviousPage
...
; an HP bar e pixels long.
	ld a, e
	cp 24				; make hp bar green until 50% as in other games
	ld d, 0 ; green
	jr nc, .gotColor
	cp 10				; HP becomes red when HP is < 20.83%
	inc d ; yellow
	jr nc, .gotColor
	inc d ; red
.gotColor
...
; Return carry on success.
	...
	call GetItemName				; call these every time so that the item name is always available in wcf4b
	call CopyStringToCF4B			; call these every time so that the item name is always available in wcf4b
	pop af							; restore the carry flag
	ret

GivePokemon::
; Give the player monster whose species is in wMonSpeciesTemp/wMonSpeciesTemp + 1 at level c.
	ld a, c
	ld [wCurEnemyLVL], a
	xor a ; PLAYER_PARTY_DATA
	ld [wMonDataLocation], a
...
const_value = 1
	...
	add_tx_pre ElevatorCantBeCalledText             ; 43
