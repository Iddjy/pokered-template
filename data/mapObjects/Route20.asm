Route20_Object:
	db $43 ; border block

	db 2 ; warps
	warp 48, 5, 0, SEAFOAM_ISLANDS_1F
	warp 58, 9, 2, SEAFOAM_ISLANDS_1F

	db 2 ; signs
	sign 51, 7, 11 ; Route20Text11
	sign 57, 11, 12 ; Route20Text12

	db 10 ; objects
	object SPRITE_SWIMMER, 87, 8, STAY, UP, 1, SWIMMER, 9
	object SPRITE_SWIMMER, 68, 11, STAY, UP, 2, BEAUTY, 14					; decreased by one since one unused trainer was removed
	object SPRITE_SWIMMER, 45, 10, STAY, DOWN, 3, BEAUTY, 6
	object SPRITE_SWIMMER, 55, 14, STAY, RIGHT, 4, JR_TRAINER_F, 23			; decreased by one since one unused trainer was removed
	object SPRITE_SWIMMER, 38, 13, STAY, DOWN, 5, SWIMMER, 10
	object SPRITE_SWIMMER, 87, 13, STAY, UP, 6, SWIMMER, 11
	object SPRITE_BLACK_HAIR_BOY_1, 34, 9, STAY, UP, 7, BIRD_KEEPER, 11
	object SPRITE_SWIMMER, 25, 7, STAY, UP, 8, BEAUTY, 7
	object SPRITE_SWIMMER, 24, 12, STAY, DOWN, 9, JR_TRAINER_F, 15			; decreased by one since one unused trainer was removed
	object SPRITE_SWIMMER, 15, 8, STAY, UP, 10, BEAUTY, 8

	; warp-to
	warp_to 48, 5, ROUTE_20_WIDTH ; SEAFOAM_ISLANDS_1F
	warp_to 58, 9, ROUTE_20_WIDTH ; SEAFOAM_ISLANDS_1F
