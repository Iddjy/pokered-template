PokemonTower4F_Object:
	db $1 ; border block

	db 2 ; warps
	warp 3, 9, 0, POKEMON_TOWER_5F
	warp 18, 9, 1, POKEMON_TOWER_3F

	db 0 ; signs

	db 6 ; objects
	object SPRITE_MEDIUM, 5, 10, STAY, RIGHT, 1, CHANNELER, 4	; decreased by 5 since 5 unused trainers were removed
	object SPRITE_MEDIUM, 15, 7, STAY, DOWN, 2, CHANNELER, 5	; decreased by 5 since 5 unused trainers were removed
	object SPRITE_MEDIUM, 14, 12, STAY, LEFT, 3, CHANNELER, 6	; decreased by 6 since 6 unused trainers were removed
	object SPRITE_BALL, 12, 10, STAY, NONE, 4, ELIXER
	object SPRITE_BALL, 9, 10, STAY, NONE, 5, AWAKENING
	object SPRITE_BALL, 12, 16, STAY, NONE, 6, HP_UP

	; warp-to
	warp_to 3, 9, POKEMON_TOWER_4F_WIDTH ; POKEMON_TOWER_5F
	warp_to 18, 9, POKEMON_TOWER_4F_WIDTH ; POKEMON_TOWER_3F
