...
.outOfBattleMovePointers
	...
	dw .softboiled
.fly
	bit THUNDER_BADGE, a ; does the player have the Thunder Badge?
...
.canFly
	...
	jp StartMenu_Pokemon
.cut
	bit CASCADE_BADGE, a ; does the player have the Cascade Badge?
	jp z, .newBadgeRequired
	predef UsedCut
	...
	jp CloseTextDisplay
.surf
	bit SOUL_BADGE, a ; does the player have the Soul Badge?
	jp z, .newBadgeRequired
	...
	jp .goBackToMap
.strength
	bit RAINBOW_BADGE, a ; does the player have the Rainbow Badge?
	jp z, .newBadgeRequired
	predef PrintStrengthTxt
	call GBPalWhiteOutWithDelay3
	jp .goBackToMap
.flash
	bit BOULDER_BADGE, a ; does the player have the Boulder Badge?
	jp z, .newBadgeRequired
	ld a, [wMapPalOffset]
	cp $06
	jr nz, .notInDarkness
	...
	ld [wMapPalOffset], a
	ld hl, wd730
	set BIT_FLASH_ACTIVE, [hl]
.notInDarkness
	callab UpdateEncounterTable	; add this to make dark maps use 2 encounter tables: 1 for when it's dark, another for when it's lit
...
DrawTrainerInfo:
	...
	ld c, $e3
	call PrintBCDNumber
	coord hl, 7, 6			; move play time value 2 slots to the left
	ld de, wPlayTimeHours ; hours
	lb bc, 2, 5				; use 2 bytes, 5 digits for play time hours, and right align it
	call PrintNumber
...
.swappingDifferentMons
	...
	ld e, l
	ld a, [wCurrentMenuItem]
	add a								; to handle 2 bytes species IDs
	add l
	ld l, a
	jr nc, .noCarry
	inc h
...
.noCarry2
	...
	ld [hSwapTemp], a					; to handle 2 bytes species IDs
	ld a, [de]							; to handle 2 bytes species IDs
	ld [hl], a							; to handle 2 bytes species IDs
	ld a, [hSwapTemp]					; to handle 2 bytes species IDs
