HandleItemListSwapping:
	...
	jp z, SwapUnquantifiableItems			; add this to handle TMs and Key items
	jp c, DisplayListMenuIDLoop				; to handle single page items menus
	push hl
	ld hl, wListPointer
	ld a, [hli]
	ld h, [hl]
...
; added this function to handle item swapping in the TMs and Key items pockets
; this function must preserve hl and de
SwapUnquantifiableItems:
	push hl
	push de
	call UpdateListAddress
	ld hl, wListPointer
	ld a, [hli]
	ld h, [hl]
	ld l, a
	...
	inc a
	jp z, DisplayListMenuIDLoop				; ignore attempts to swap the Cancel menu item
	ld a, [wMenuItemToSwap]					; index of item chosen for swapping (counts from 1)
	and a									; has the first item to swap already been chosen?
	jr nz, .swapItems
; if not, set the currently selected item as the first item
	ld a, [wCurrentMenuItem]
	...
	ld a, [wListScrollOffset]				; index of top (visible) menu item within the list
	add b
	ld [wMenuItemToSwap], a					; index of item chosen for swapping (counts from 1)
	ld c, 20
	call DelayFrames
	jp DisplayListMenuIDLoop
.swapItems
...
.noCarry									; de = address of first item to swap
	...
	ld [$ff95], a							; [$ff95] = second item ID
	ld a, [de]
	ld [hl], a								; put first item ID in second item slot
	ld a, [$ff95]
	ld [de], a								; put second item ID in first item slot
	xor a
	ld [wMenuItemToSwap], a					; 0 means no item is currently being swapped
