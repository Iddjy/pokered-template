...
; add a wrapper function with the same name as the original one to avoid having to update all references to it
RemoveItemByID:
	xor a
	ld [hItemToRemoveIndex], a			; moved here from the original function
	ld c, a								; initialize index counter
	...
	ld a, [wIsKeyItem]
	and a
	jp z, RemoveQuantifiableItemByID	; if we're removing a non-key item, use the original function
	ld hl, wKeyItemsList
.loop
	ld a, [hli]
