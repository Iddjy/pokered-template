...
; [wMonPartySpriteSpecies].
	...
	ld b, a
	ld a, [wMonPartySpriteSpecies + 1]	; to handle 2 bytes species IDs
	ld c, a
	call GetPartyMonSpriteID
	ld [wOAMBaseTile], a
...
; use bc as input instead of a to handle 2 bytes species IDs
GetPartyMonSpriteID:
	ld d, c					; save the lower byte in d for later
	dec bc
	srl b
	rr c
	ld hl, MonPartyData
	add hl, bc
	ld a, [hl]
	bit 0, d				; check parity of lower byte
	jr nz, .skipSwap
