PrintBeginningBattleText:
	...
	jr z, .isMarowak
	ld a, [wEnemyMonType1]
	cp GHOST							; use the pokemon type to determine whether it's a GHOST, instead of the map we're in
	jr z, .isGhost
	ld a, [wEnemyMonType2]
	cp GHOST
	jr nz, .playCry
...
.wildBattle
	push hl
	ld a, 2							; make the pokeballs use the red palette
	ld hl, wPlayerHPBarColor
	ld [hli], a
	ld [hl], a						; also set wEnemyHPBarColor to make the enemy's balls red too
	ld b, SET_PAL_BATTLE
