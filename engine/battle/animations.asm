...
.foundMatch
	...
	push hl
	push de
	call GetMoveSound			; c must not be modified between the call to GetMoveSound and the call to PlayCrySound
	ld b, a						; because PlayCrySound assumes c holds the base cry id's high byte
	call IsCryMove				; and GetMoveSound puts it in c as its output
	ld a, b
	jr nc, .notCryMove
	call PlayCrySound
	jr .afterPlaySound
...
SpecialEffectPointers:
	...
	db SE_REVERSE_TURN
	dw ReverseTurn
	db SE_DARKEN_MON_PALETTE_2
	dw AnimationDarkenMonPalette2
	db SE_VERY_LIGHT_SCREEN_PALETTE
	dw AnimationVeryLightScreenPalette
	db SE_BLACK_SQUARES_EVERYWHERE
	dw AnimationBlackSquaresEverywhere
	db $FF
...
AnimationDarkenMonPalette2:
; Darkens the mon sprite's palette even more
	lb bc, $fe, $f8
	jr SetAnimationBGPalette
...
AnimationVeryLightScreenPalette:
	lb bc, $40, $40
...
AnimationBlackSquaresEverywhere:
	ld a, 1
	ld [wWhichBattleAnimTileset], a
	call LoadAnimationTileset
	ld b, $74							; black tile
	ld d, 24							; duration of the animation in frames
	jr AnimationWaterDropletsEverywhere.animationTileEverywhere
...
; Minimize animation.
	call LoadMiniSprite			; replace the body of the function by this call to avoid code duplication
	call Delay3
	jp AnimationShowMonPic
...
.next2
	...
	and a
	jp nz, LoadMiniSprite			; if it is, load mini sprite instead of mon pic
	ld a, [de]
...
.continue
	...
	call GetCryData					; after GetCryData, bc contains the base cry id
	pop hl
	ld d, a
...
MoveSoundTable:
	...
	db SFX_PSYCHIC_M,         $30,$10 ; FAIRY_WIND
	db SFX_PSYCHIC_M,         $05,$f0 ; MOONBLAST
...
CallHideSubstituteShowMonAnim_:
	ld a, [H_WHOSETURN]
	and a
...
.main
	...
	call nz, HideSubstituteShowMonAnim
	ret

CallReshowSubstituteAnim_:
	ld a, [H_WHOSETURN]
	and a
	ld a, [wPlayerBattleStatus2]
	jr z, .main
