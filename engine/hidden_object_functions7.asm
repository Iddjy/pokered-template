...
.billsPokemonLoop
	...
	jr nz, .cancel
	ld a, [wCurrentMenuItem]
	cp 4								; just check if player chose cancel and jump if they did, instead of checking each other options
	jr z, .cancel
	ld hl, EEVEE						; to handle 2 bytes species IDs (even though with EEVEE and its evolutions it's not needed)
	ld c, a								; to handle 2 bytes species IDs
	ld b, 0								; to handle 2 bytes species IDs
...
BillsMonListText:
	db   "EEVEE"
	next "VAPOREON"				; rearranged them in pokedex order
	next "JOLTEON"
	next "FLAREON"				; rearranged them in pokedex order
	next "CANCEL@"
...
; for the Battle Facility Elevator in Silph Co.
PrintElevatorCantBeCalledText:
	call EnableAutoTextBoxDrawing
	tx_pre_jump ElevatorCantBeCalledText

; for the Battle Facility Elevator in Silph Co.
ElevatorCantBeCalledText:
	TX_FAR _ElevatorCantBeCalledText
	db "@"
