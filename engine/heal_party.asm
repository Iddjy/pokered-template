...
.healmon
	...
	cp $ff						; to handle 2 bytes species IDs
	ld [wMonSpeciesTemp], a		; for ReadSignatureMoveData later
	ld a, [hli]					; to handle 2 bytes species IDs
	ld [wMonSpeciesTemp + 1], a	; for ReadSignatureMoveData later
	jr nz, .notDone				; to handle 2 bytes species IDs
	cp $ff
	jr z, .done
.notDone						; to handle 2 bytes species IDs
