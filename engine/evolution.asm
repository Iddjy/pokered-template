EvolveMon:
	...
	ld a, [wEvoOldSpecies + 1]					; to handle 2 bytes species ID
	...
	ld a, [wEvoNewSpecies + 1]					; to handle 2 bytes species ID
	...
	ld c, 80
	call DelayFrames
	ld c, PAL_BLACK					; set PAL_BLACK instead of mon palette
	call EvolutionSetWholeScreenPalette
	lb bc, $1, $10
.animLoop
	push bc
	...
	ld hl, wEvoNewSpecies						; to handle 2 bytes species ID
.done
	...
	ld b, a										; to handle 2 bytes species ID
	ld a, [wWholeScreenPaletteMonSpecies + 1]	; to handle 2 bytes species ID
	ld c, a										; to handle 2 bytes species ID
	call PlayCry
	ld c, 0
	call EvolutionSetWholeScreenPalette
...
.evolutionCancelled
	...
	ld a, 1
	ld [wEvoCancelled], a
	ld hl, wEvoOldSpecies						; to handle 2 bytes species ID
	jr .done
