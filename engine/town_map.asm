...
.townMapLoop
	push bc							; save b which contains info on which button was pressed
	coord hl, 0, 0
	lb bc, 1, 20
	...
	add hl, bc
	ld a, [hl]
	ld d, a							; save map ID in d
	pop bc							; restore state of button presses in b
	call SkipUndiscoveredLocations	; if location is hidden, sets the z flag
	ld a, d							; restore map ID in a for LoadTownMapEntry
	jr z, .checkButtonPress			; if location is hidden, skip it by reapplying the same button press again
.enterLoop
...
; allows the cursor to skip certain locations according to event flags
; input: map id in a
; output: sets z flag if location must be skipped, unsets it otherwise
SkipUndiscoveredLocations:
; example of code to handle 2 possible maps that can be skipped
;	cp MAP_ID_1
;	jr nz, .checkMapId2
;	CheckEvent EVENT_VISITED_MAP_ID_1
;	ret
;.checkMapId2
;	cp MAP_ID_2
;	ret nz
;	CheckEvent EVENT_VISITED_MAP_ID_2
	or 1								; default implementation just unsets z flag, remove this line if you implement actual checks like the example above
	ret
...
LoadTownMap_Nest:
	...
	ld de, ScanningText
	coord hl, 1, 0
	call PlaceString				; display SCANNING... text at the top to let player know something is happening
	ld hl, wUpdateSpritesEnabled
	ld a, [hl]
	push af
	ld [hl], $ff
	...
	coord hl, 1, 0
	lb bc, 1, 17
	call ClearScreenArea			; remove SCANNING... text
	call GetMonNameByDexNumber		; to handle species with multiple forms
	coord hl, 1, 0
	call PlaceString
	ld h, b
	ld l, c
...
.loop
	...
	push de
	push hl
	ld hl, HiddenLocations
	ld de, 0001
	call IsInArray
	pop hl
	pop de
	jr c, .nextEntry			; skip hidden locations
	ld a, c						; IsInArray puts a in c and clobbers a, so restore it for next function
	call TownMapCoordsToOAMCoords
...
; list of town map coords of locations for which we don't show nests in the pokedex
HiddenLocations:
	db $19			; Cerulean Cave
	db $FF
