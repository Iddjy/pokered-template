...
; In-game trades and internally clocked link cable trades use this.
	...
	ld a, [wTradedPlayerMonSpecies + 1]			; to handle 2 bytes species IDs
	ld [wLeftGBMonSpecies + 1], a				; to handle 2 bytes species IDs
	ld a, [wTradedEnemyMonSpecies]
	ld [wRightGBMonSpecies], a
	ld a, [wTradedEnemyMonSpecies + 1]			; to handle 2 bytes species IDs
	ld [wRightGBMonSpecies + 1], a				; to handle 2 bytes species IDs
...
InternalClockTradeFuncSequence:
	tradefunc LoadTradingGFX
	tradefunc LoadMonNames
...
TradeFuncPointerTable:
	addtradefunc LoadTradingGFX
	addtradefunc LoadMonNames
...
.next
	...
	xor a
	ld [H_AUTOBGTRANSFERENABLED], a
	ret									; just load GFX in this function

; make this part into a dedicated function
LoadMonNames:
	ld a, [wTradedPlayerMonSpecies]
	...
	call GetMonName
	ld hl, wcd6d
	ld de, wcf4b								; save player's mon's name in cf4b
	ld bc, NAME_LENGTH
	call CopyData
	ld a, [wTradedEnemyMonSpecies]
	ld [wMonSpeciesTemp], a						; to handle 2 bytes species IDs
	ld a, [wTradedEnemyMonSpecies + 1]			; to handle 2 bytes species IDs
	ld [wMonSpeciesTemp + 1], a					; to handle 2 bytes species IDs
	jp GetMonName								; put other gameboy's mon's name in cd6d
...
Trade_ShowEnemyMon:
	...
	call PrintTradeTakeCareText
	ld a, [wTradedMonIsNewDexEntry]
	and a
	ret z
	ld hl, NewDexDataAddedText4
	call PrintText
	predef ShowPokedexData
	call LoadHpBarAndStatusTilePatterns			; reload correct tileset for trade animation (needed for the "ID" and "N°" characters)
	jp LoadTradingGFX							; reload trading GFX after displaying pokedex
...
; Animates the mon moving from the left GB to the right one.
	...
	ld a, [wLeftGBMonSpecies + 1]					; to handle 2 bytes species IDs
	ld [wMonPartySpriteSpecies + 1], a				; to handle 2 bytes species IDs
...
; Animates the mon moving from the right GB to the left one.
	...
	ld [wMonPartySpriteSpecies], a
	ld a, [wRightGBMonSpecies + 1]					; to handle 2 bytes species IDs
	ld [wMonPartySpriteSpecies + 1], a				; to handle 2 bytes species IDs
	call Trade_WriteCircledMonOAM
	call Trade_DrawRightGameboy
	call Trade_CopyTileMapToVRAM
