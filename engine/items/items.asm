UseItem_:
	...
	ld b, 0
	jr nc, .next
	ld b, 1				; new items may have IDs beyond 127, so we must check for carry
.next
	add hl, bc
	ld a, [hli]
	ld h, [hl]
...
ItemUsePtrTable:
	...
	dw ItemUseBattlePass ; BATTLE_PASS
	...
	dw ItemUseXStat      ; X_SP.ATK
	dw ItemUseXStat      ; X_SP.DEF
	dw ItemUseXStat      ; X_ACCURACY	; changed from ItemUseXAccuracy to make X_ACCURACY behave like other X items, and moved it here so
										; that it's right after X_SP.DEF which is needed for the right move effect to be used
	...
	dw ItemUseOriginPlate; ORIGIN_PLATE
	...
	dw UnusableItem      ; GIFT_CARD
	dw ItemUseBerry      ; POMEG_BERRY 
	dw ItemUseBerry      ; KELPSY_BERRY
	dw ItemUseBerry      ; QUALOT_BERRY
	dw ItemUseBerry      ; HONDEW_BERRY
	dw ItemUseBerry      ; GREPA_BERRY 
	dw ItemUseBerry      ; TAMATO_BERRY
	dw UnusableItem      ; BOTTLE_CAP
	dw ItemUseGriseousCore	; GRISEOUS_CORE
	dw UnusableItem      ; GOLDBOTTLECAP
...
; Pokemon can't be caught and skip the capture calculations.
	ld a, [wCurOpponent]
	ld b, a								; to handle 2 bytes species ID
	ld a, [wCurOpponent + 1]			; to handle 2 bytes species ID
	ld c, a								; to handle 2 bytes species ID
	ld hl, GHOST_MAROWAK				; to handle 2 bytes species ID
	call CompareBCtoHL					; to handle 2 bytes species ID
	ld b, $10 ; can't be caught value
	jp z, .setAnimData
...
; Mirror Move even though the only wild Pokémon that knows Transform is Ditto.
	...
	ld a, [wTransformedEnemyMonOriginalSpecies + 1]	; species ID are now 2 bytes
...
; new DVs.
	...
	ld [hli], a
	ld a, [wEnemyMonDVs + 1]
	ld [hli], a					; add this for the new DV for special defense
	ld a, [wEnemyMonDVs + 2]	; add this for the new DV for special defense
	ld [hl], a
.skip6
	ld a, [wEnemyMonLevel]
	ld [wCurEnemyLVL], a
...
; Add the caught Pokémon to the Pokédex.
	...
	call CheckPokedexOwned				; to handle 2 bytes species IDs
	jr nz, .skipShowingPokedexData		; if already registered, don't show the Pokédex data
...
ItemUseEvoStone:
	...
	xor a
	ld [wEvoStoneItemID], a			; reset this variable after use
...
; if it is active in battle
	...
	ld bc, NUM_STATS * 2
	call FullHealUpdateStats			; add this to recalculate stats correctly after healing a status with Full Heal
	jp .doneHealing
.healHP
...
.fullHP ; if the pokemon's current HP equals its max HP
	...
	dec hl
	dec hl
	dec hl								; to handle 2-bytes species IDs
...
.notUsingSoftboiled2
	ld a, [wcf91]
	cp SODA_POP
	ld b, SODA_POP_HEAL_AMOUNT
	jr z, .addHealAmount
	ld b, LEMONADE_HEAL_AMOUNT
	jr nc, .addHealAmount
	cp FRESH_WATER
	ld b, FRESH_WATER_HEAL_AMOUNT
	jr z, .addHealAmount
	cp SUPER_POTION
...
; copy party HP to in-battle HP
	...
	ld [wBattleMonStatus], a	; remove the status ailment in the in-battle pokemon data
	push de
	call FullRestoreUpdateStats	; add this to recalculate stats correctly after healing a status with Full Restore
	pop de
...
.useVitamin
	...
	push hl
	sub HP_UP
	ld bc, wPartyMon1HPEV - wPartyMon1
	add hl, bc							; make hl point to HP EV
	ld c, a								; store item id - HP_UP in c
	ld b, 0
	add hl, bc							; make hl point to EV corresponding to vitamin used
	push hl
	callab checkGlobalEVLimit			; check if mon's total EVs are >= MAX_EV_GLOBAL
	pop hl
	ld a, e								; e contains high byte of the difference between the mon's total EVs and MAX_EV_GLOBAL
	and a
	ld c, VITAMIN_BONUS_AMOUNT + 1		; if high byte of the difference is not null, set c higher than VITAMIN_BONUS_AMOUNT
	jr nz, .farFromMaxed				; if the difference between total EVs and global limit is > 255, don't check low byte
	ld c, d								; store the low byte of the difference in c
	ld a, c
	and a
	jr z, .vitaminNoEffect				; if low byte is also zero, then it means EVs are already maxed out
.farFromMaxed
	ld a, VITAMIN_BONUS_AMOUNT
	ld b, a
	cp c								; check if the difference between total EVs and global limit is lower than VITAMIN_BONUS_AMOUNT
	jr c, .addEV
	ld b, c								; if EV gain from vitamin would make EV total go over global limit, set the EV gain to the difference
.addEV
	ld a, [hl]							; a = current EV of the appropriate stat
	cp MAX_EV_PER_STAT
	jr nc, .vitaminNoEffect				; if current EV is already maxed, vitamin has no effect
	add b								; add EVs
	jr c, .maxEV						; in case EV gets over 255, prevents rollover
	cp MAX_EV_PER_STAT
	jr nc, .maxEV						; if new value is over the limit, cap it to the limit
	ld [hl], a
	jr .updateEVDone
.maxEV
...
.recalculateStats
	...
	ld bc, (wPartyMon1HPEV - 1) - wPartyMon1Stats
	add hl, bc
	ld b, 1
...
.useRareCandy
	...
	ld a, [hl]										; a = level
	cp MAX_LEVEL
	jr z, .vitaminNoEffect							; can't raise level above MAX_LEVEL
	inc a
...
ItemUseEscapeRope:
	...
	cp AGATHAS_ROOM
	jr z, .notUsable
	cp SAFFRON_GYM					; Saffron Gym uses the FACILITY tileset
	jr z, .notUsable				; so we have to add an exception for it
	cp CINNABAR_GYM					; Cinnabar Gym uses the FACILITY tileset
	jr z, .notUsable				; so we have to add an exception for it
	cp BILLS_HOUSE					; Bill's House uses the INTERIOR tileset
	jr z, .notUsable				; so we have to add an exception for it
	cp POKEMON_FAN_CLUB				; Pokemon Fan Club uses the INTERIOR tileset
	jr z, .notUsable				; so we have to add an exception for it
	call IsInteriorMap				; use new function to save space
	jr z, .notUsable
	ld hl, wd72e
	res 4, [hl]
	...
	ld hl, wd732
	set 3, [hl]
	call CheckSecretPassages
	jr c, .skipLastBlackoutMapFlag
	set 6, [hl]
.skipLastBlackoutMapFlag
	ld a, [wPseudoItemID]
	and a ; using Dig?
	ret nz ; if so, return
	call ItemUseReloadOverworldData
...
EscapeRopeTilesets:
	db FOREST, CEMETERY, CAVERN, FACILITY, INTERIOR, UNDERGROUND
	db $ff ; terminator

; add this for potential GHOSTs in exterior maps, to adapt text
; sets the z flag if map is not an interior map, unsets it otherwise
IsInteriorMap::
	ld a, [wCurMapTileset]
	ld b, a
...
; check for secret passages when using Escape Rope or Dig
; sets carry flag if secret passage found, unsets it otherwise
CheckSecretPassages:
	push hl
	ld a, [wCurMap]
	ld b, a
	...
	ld a, [wYCoord]
	ld e, a
	ld hl, DigSecretPassages
	ld a, [wPseudoItemID]
	and a							; using Dig?
	jr nz, .loop					; if so, jump
	ld hl, EscapeRopeSecretPassages
.loop
	ld a, [hli]
	cp $ff
	jr z, .done
	cp b
	jr z, .checkXCoordinate
	inc hl
	inc hl
	inc hl
	inc hl
...
.secretPassageFound
	...
	set 7, [hl]
	xor a
	ld [wLastMusicSoundID], a		; needed to force the music to restart after entering the target map
	scf
.done
	pop hl
	ret

; list of secret passages accessible by using an Escape Rope in certain spots on certain maps
; format: source map ID, secret passage X coordinate, secret passage Y coordinate, target map ID, wLastMap override ($FF means don't override), source map dungeon warp ID
; target coordinates are defined in DungeonWarpData in data/special_warps.asm
; wLastMap needs to be overridden when the secret passage's target map has a different parent map from the source map, otherwise when exiting the target map the game will try to load the wrong map.
EscapeRopeSecretPassages:
;	db POKEMON_MANSION_2F, 12, 16, POKEMON_MANSION_3F, $FF, 5	; example
	db $ff

; list of secret passages accessible by using Dig in certain spots on certain maps
; format: source map ID, secret passage X coordinate, secret passage Y coordinate, target map ID, wLastMap override ($FF means don't override), source map dungeon warp ID
; target coordinates are defined in DungeonWarpData in data/special_warps.asm
; wLastMap needs to be overridden when the secret passage's target map has a different parent map from the source map, otherwise when exiting the target map the game will try to load the wrong map.
DigSecretPassages:
...
ItemUseGuardSpec:
	...
	and a
	jp z, ItemUseNotTime
	ld hl, wPlayerMistCounter			; check the counter instead of the flag
	ld a, [hl]
	and a
	jp nz, ItemUseNoEffect				; if counter is not zero, player is already protected by Mist, so item has no effect
...
.inBattle
	...
	push hl
	ld a, [wcf91]
	sub X_ATTACK - ATTACK_UP2_EFFECT		; use double stat up effects to make X items behave like in modern generations
	...
	callab CallHideSubstituteShowMonAnim
...
ItemUseOldRod:
	call FishingInit
	jp c, ItemUseNotTime
	ld d, 5							; set the level
	ld bc, MAGIKARP					; handle 2 bytes species IDs
	ld a, $1 ; set bite
	jr RodResponse
...
.RandomLoop
	...
	ld hl, GoodRodMons
	ld c, a					; store random number in c
	add a					; double random number in a
	add c					; a = 3 * random number (each entry in GoodRodMons is now 3 bytes with 2 bytes species IDs)
	ld c, a
	ld b, 0
	add hl, bc				; make hl point to correct entry in GoodRodMons
	ld d, [hl]				; store the level in d
	inc hl
	ld c, [hl]
	inc hl
	ld b, [hl]				; store the species ID in bc
	and a
.SetBite
	ld a, 0
	rla
...
RodResponse:
	...
	ld a, d						; level
	ld [wCurEnemyLVL], a
	ld a, b						; species in bc
	ld [wCurOpponent], a
	ld a, c
	ld [wCurOpponent + 1], a	; to handle 2 bytes species IDs
	xor a
	ld [wEngagedTrainerClass], a; clear wEngagedTrainerClass because it is clobbered when talking to a NPC offering an in-game trade
...
; add this to toggle Exp. Share activation
ItemUseExpShare:
	ld hl, wOptions
	bit EXP_SHARE_STATE, [hl]
	ld hl, ExpShareDeactivatedText
	jr z, .next
	ld hl, ExpShareActivatedText
.next
	call PrintText
	coord hl, 13, 9
	lb bc, 10, 14
	...
	ret z							; if the player chose NO, don't toggle Exp. Share
	ld hl, wOptions
	ld a, [hl]
	xor (1 << EXP_SHARE_STATE)		; toggle the exp share bit
	ld [hl], a
	bit EXP_SHARE_STATE, a
	ld hl, ActivateExpShareText
	jr nz, .displayText
	ld hl, DeactivateExpShareText
.displayText
	call PrintText
	ret

ExpShareDeactivatedText:
	TX_FAR _ExpShareDeactivatedText
	db "@"

ExpShareActivatedText:
	TX_FAR _ExpShareActivatedText
	db "@"

DeactivateExpShareText:
	TX_FAR _DeactivateExpShareText
	db "@"

ActivateExpShareText:
	TX_FAR _ActivateExpShareText
	db "@"
...
; PP if the move has had any PP Ups used on it.
	and %00111111		; fix bug described above
	cp b ; does current PP equal max PP?
	ret z
	jr .storeNewAmount
.useElixir
...
; add this to allow the Player to change Arceus's Type
ItemUseOriginPlate:
	ld a, [wIsInBattle]
	and a
	jp nz, ItemUseNotTime
...
.loop
	...
	ld bc, wPartyMon2Type1 - wPartyMon1Type1
	call AddNTimes
	ld a, [hli]
	ld b, a
	ld a, [hld]
	...
	ld e, l
	ld a, [wcf91]
	sub NORMAL_TYPE				; map item id to type id
	ld hl, AlreadyTypeText
	cp b
	jr nz, .changeType
	cp c
	jr z, .printText
.changeType
	...
	inc b
.noCarry
	ld hl, wPartyMon1Species - wPartyMon1Type2
	add hl, de
	ld a, b
	ld [hli], a									; change the species id in party data
	ld a, c
	ld [hl], a									; change the species id in party data
	ld hl, wPartySpecies
	ld a, [wWhichPokemon]
	add a										; double index to account for 2-bytes species IDs
	add l
	ld l, a
	jr nc, .noCarry2
	inc h
.noCarry2
	ld a, b
	ld [hli], a									; change the species id in party list
	ld a, c
	ld [hl], a									; change the species id in party list
	ld a, SFX_HEAL_AILMENT
	call PlaySoundWaitForCurrent
	ld hl, BecameTypeText
.printText
	call PrintText
.done
...
WhichTypeText:
	TX_FAR _WhichTypeText
	db "@"

BecameTypeText:
	TX_FAR _BecameTypeText
	db "@"

AlreadyTypeText:
	TX_FAR _AlreadyTypeText
	db "@"
...
RestoreBonusPP:
	ld hl, wPartyMon1						; start at the species id
	ld bc, wPartyMon2 - wPartyMon1
	ld a, [wWhichPokemon]
	call AddNTimes
	ld a, [hli]								; put the species id in wMonSpeciesTemp/wMonSpeciesTemp+1 for LoadMovePPs
	ld [wMonSpeciesTemp], a					; put the species id in wMonSpeciesTemp/wMonSpeciesTemp+1 for LoadMovePPs
	ld a, [hld]								; put the species id in wMonSpeciesTemp/wMonSpeciesTemp+1 for LoadMovePPs
	ld [wMonSpeciesTemp + 1], a				; put the species id in wMonSpeciesTemp/wMonSpeciesTemp+1 for LoadMovePPs
	ld bc, wPartyMon1Moves - wPartyMon1		; make hl point to wPartyMon1Moves
	add hl, bc								; make hl point to wPartyMon1Moves
	push hl
...
.loop
	inc b
	ld a, b
	cp NUM_MOVES + 1			; reached the end of the pokemon's moves?
	ret z						; if so, return
	ld a, [wUsingPPUp]
	dec a ; using a PP Up?
...
.signatureMove
	ld [wd11e], a					; for ReadSignatureMoveData
	ld a, [wMonDataLocation]
	and a
	ld hl, wPartyMon1Species
	ld bc, wPartyMon2 - wPartyMon1
...
.multipleMon
	ld a, [wWhichPokemon]
	call AddNTimes
.readSpeciesId						; put the species id in wMonSpeciesTemp/wMonSpeciesTemp+1 for ReadSignatureMoveData
	ld a, [hli]
	ld [wMonSpeciesTemp], a
	ld a, [hl]
	ld [wMonSpeciesTemp + 1], a
	callab ReadSignatureMoveData
	jr .afterMoveDataFetch
.notSignatureMove
	dec a	
	ld hl, Moves
	ld bc, MoveEnd - Moves
...
IsKeyItem_:
	...
	ret nc					; consider TMs as Key Items too
; if the item is not an HM or TM
	push af
	ld hl, KeyItemBitfield
	ld de, wBuffer
	ld bc, 22				; now we need 22 bytes to account for all the new items
	call CopyData
	pop af
	dec a
	ld c, a
...
.boxNotFilled
	ld a, [wMonSpeciesTemp]			; species ID of caught mon
	ld b, a
	ld a, [wMonSpeciesTemp + 1]		; species ID of caught mon
	ld c, a
.insertSpeciesIdInBoxSpeciesList
; in this loop, bc holds the species ID we're inserting and hl is used to save the overwritten species ID
	inc de
	ld a, [de]
	ld h, a										; save current value in h for next iteration
	ld a, b										; shift species list 2 bytes up
	ld [de], a									; shift species list 2 bytes up
	inc de										; shift species list 2 bytes up
	ld a, [de]									; shift species list 2 bytes up
	ld l, a										; save current value in l for next iteration
	ld a, c										; shift species list 2 bytes up
	ld [de], a									; shift species list 2 bytes up
	push hl										; swap hl and bc for next iteration
	push bc										; swap hl and bc for next iteration
	pop hl										; swap hl and bc for next iteration
	pop bc										; swap hl and bc for next iteration
	ld a, b										; check if we reached the terminator
	cp $ff										; check if we reached the terminator
	jr nz, .insertSpeciesIdInBoxSpeciesList		; if at least one byte is not $ff, it's not the terminator
	ld a, c										; check if we reached the terminator
	cp $ff										; check if we reached the terminator
	jr nz, .insertSpeciesIdInBoxSpeciesList		; if at least one byte is not $ff, it's not the terminator
	inc de										; put the terminator at the end
	ld a, $ff									; put the terminator at the end
	ld [de], a									; put the terminator at the end
	inc de										; put the terminator at the end
	ld [de], a									; put the terminator at the end
	call GetMonHeader
	ld hl, wBoxMonOT
...
.initLevelAndOTID
	...
	ld hl, wEnemyMon
	ld de, wBoxMon1
	ld bc, (wEnemyMonMoves + NUM_MOVES) - wEnemyMon
	call CopyData										; copy data up to and including the moveset
	ld hl, wPlayerID
	ld a, [hli]
	ld [de], a
	inc de
	...
	inc de
	xor a
	ld b, NUM_STATS			; removed multiplication by 2 since now EVs are stored over 1 byte only
.initEVs
	ld [de], a
	inc de
	dec b
...
IsNextTileShoreOrWater:
	...
	cp FACILITY
	jr z, .skipShoreTiles	; also ignore shore tiles in the Facility tileset
...
.RandomLoop
	...
	and %1111 ; 4-bit random number (allow up to 16 different slots)
	cp b
	...
	ld c, a					; store random number in c
	add a					; double random number in a
	add c					; a = 3 * random number (each entry in SuperRodData is now 3 bytes with 2 bytes species IDs)
	ld c, a
	ld b, $0
	add hl, bc
	ld d, [hl]				; level
...
FullRestoreUpdateStats:
	ld hl, wPartyMon1
	ld bc, wPartyMon2 - wPartyMon1
	ld a, [wPlayerMonNumber]
	call AddNTimes						; makes hl point to party data of the healed pokemon
	ld bc, wPartyMon1MaxHP - wPartyMon1
	add hl, bc							; hl now points to party stats
...
FullHealUpdateStats:
	call CopyData
	xor a
	ld [H_WHOSETURN], a		; force the turn to the player's since this part is only for items used by the player
DoUpdateStats:
	jpab UpdateStats

; function when using the BATTLE PASS from the bag
ItemUseBattlePass:
	ld hl, BattlePassUsageText
	jp PrintText

BattlePassUsageText:
	TX_FAR _BattlePassUsageText
	db "@"

; function for EV reducing berries
ItemUseBerry:
	ld a, [wIsInBattle]
	and a
	jp nz, ItemUseNotTime
	...
	call AddNTimes
	ld a, [hl]
	ld [wCurEnemyLVL], a				; need to set this variable for recalculating stats later
	ld bc, wPartyMon1HPEV - wPartyMon1Level
	add hl, bc
	ld a, [wcf91]
	sub POMEG_BERRY
	ld c, a								; store item id - POMEG_BERRY in c
	ld b, 0
	add hl, bc							; make hl point to EV corresponding to vitamin used
	ld a, VITAMIN_BONUS_AMOUNT			; use new constant instead of 10
	ld b, a
	ld a, [hl]							; a = current EV of the appropriate stat
	and a
	jr z, .berryNoEffect				; if current EV is already zero, berry has no effect
	sub b								; reduce EVs
	jr c, .zeroEV						; in case EV gets negative, prevents rollover
	ld [hl], a
	jr .updateEVDone
.zeroEV
	xor a
...
.updateEVDone
	...
	ld l, e
	call CompareBCtoHL
	pop hl										; restore max HP address in hl
	pop bc										; restore current HP address in bc
	jr nc, .printText							; if current HP <= max HP, skip next part
	ld a, [hli]									; else set current HP to max HP
	ld [bc], a
	inc bc
	ld a, [hl]
	ld [bc], a
.printText
	...
	sub POMEG_BERRY - 1
	ld c, a
.statNameLoop ; loop to get the address of the name of the stat the berry decreases
	dec c
	jr z, .gotStatName
.statNameInnerLoop
	ld a, [hli]
...
ItemUseGriseousCore:
	...
	ld l, [hl]
	ld h, a
	ld bc, GIRATINA_ALTERED
	call CompareBCtoHL
	jr nz, .testOriginForme
	inc bc
	jp ChangeGiratinaForme
.testOriginForme
	ld bc, GIRATINA_ORIGIN
	call CompareBCtoHL
	jr nz, .notAble
	dec bc
	jp ChangeGiratinaForme
...
; input: old forme species ID in hl, new form species ID in bc
ChangeGiratinaForme:
	push bc
	ld a, l
	...
	ld hl, GIRATINA_ALTERED
	pop bc							; restore new forme species ID in bc
	call CompareBCtoHL
	ld de, AlteredFormeText
	jr z, .next
	ld de, OriginFormeText
.next
	ld hl, wcd6d
	call CopyString
	ld hl, ChangedFormeText
	call PrintText
	ld hl, wPartyMon1
	ld bc, wPartyMon2 - wPartyMon1
	ld a, [wWhichPokemon]
...
.noCarry2
	ld a, [wEvoNewSpecies]
	ld [hli], a									; change the species id in party list
	ld [wMonSpeciesTemp], a						; for GetMonHeader
	ld a, [wEvoNewSpecies+1]
	ld [hl], a									; change the species id in party list
	ld [wMonSpeciesTemp+1], a					; for GetMonHeader
	call GetMonHeader							; for CalcStats
	ld hl, wPartyMon1Level
	ld bc, wPartyMon2 - wPartyMon1
	ld a, [wWhichPokemon]
	call AddNTimes
	ld a, [hl]
	ld [wCurEnemyLVL], a
	ld bc, wPartyMon1 - wPartyMon1Level
	add hl, bc
	call ItemUseMedicine.recalculateStats
