...
.sellMenu
	...
	ld a, [wItemMenuCurrentPage]		; save the current page before entering the bag menu since it overwrites it
	ld [wSavedItemMenuCurrentPage], a
...
.sellMenuLoop
	...
	ld [wPrintItemPrices], a
	ld [wCurrentMenuItem], a
	ld a, ITEMLISTMENU_NOPAGES			; changed from ITEMLISTMENU to restrict it to standard items page
	ld [wListMenuID], a
	call DisplayListMenuID
	jp c, .exitFromSellMenu				; we need to restore the items page when exiting the sell menu
.confirmItemSale ; if the player is trying to sell a specific item
	...
	ld a, [wcf91]
	cp MASTER_BALL				; check if player is trying to sell a Master Ball
	jr z, .unsellableItem		; can't sell Master Ball
	call IsItemHM
	jr c, .unsellableItem
...
.unsellableItem
	...
	call PrintText
	jp .exitFromSellMenu				; we need to restore the items page when exiting the sell menu
.bagEmpty
...
.skipSettingFlag2
	...
	jp .buyMenuLoop
.exitFromSellMenu
	ld a, [wSavedItemMenuCurrentPage]
	ld [wItemMenuCurrentPage], a		; restore page number once done with the sell menu
