...
CableClub_DoBattleOrTrade:
	...
	call PlaceString
	ld hl, wUnknownSerialCounter2				; this seems like a more likely variable name to use here for the same address
	xor a
	ld [hli], a
	ld [hl], $50
	; fall through
...
; Since SERIAL_NO_DATA_BYTE has a special meaning in the context of the game link transfer protocol,
; it can't be transfered as is when present in the data, so this conversion is needed.
; And to be able to reverse it on the receiving gameboy, the patch list is used to record the positions in the data where bytes were converted
	ld a, c
	ld [de], a
	inc de
	ld [hl], $ff
...
.skipSendingTwoZeroBytes
	...
	ld bc, $1a9							; size of the player data to send/receive to/from the other gameboy
	call Serial_ExchangeBytes
	ld a, SERIAL_NO_DATA_BYTE
	ld [de], a
	ld hl, wSerialPartyMonsPatchList
...
.finishedEnemyMonsPatchListPart
	...
	ld a, SONY1
	ld [wCurOpponent + 1], a
	ld [wEngagedTrainerClass], a	; this is the variable that is checked to differentiate wild battles from trainer battles now
	call ClearScreen
	call Delay3
	ld hl, wOptions
	res 7, [hl]
...
.loop
	ld a, [de]
	cp $ff
	jr nz, .notDone						; to handle 2 bytes terminator
	inc de								; to handle 2 bytes terminator
	ld a, [de]							; to handle 2 bytes terminator
	dec de								; to handle 2 bytes terminator
	cp $ff								; to handle 2 bytes terminator
	ld a, [de]							; to handle 2 bytes terminator
	ret z
.notDone								; to handle 2 bytes terminator
...
TradeCenter_Trade:
	...
	jr z, .tradeConfirmed
; if trade cancelled
	ld a, $1												; send $1 to the other gameboy to signal trade cancellation
	ld [wSerialExchangeNybbleSendData], a
	coord hl, 0, 12
	ld b, 4
	ld c, 18
	...
	ld de, TradeCanceled
	call PlaceString
	call Serial_PrintWaitingTextAndSyncAndExchangeNybble	; send your signal and receive the signal from the other gameboy
	jp .tradeCancelled
.tradeConfirmed
	ld a, $2												; send $2 to the other gameboy to signal trade confirmation
	ld [wSerialExchangeNybbleSendData], a
	call Serial_PrintWaitingTextAndSyncAndExchangeNybble	; send your signal and receive the signal from the other gameboy
	ld a, [wSerialSyncAndExchangeNybbleReceiveData]			; check signal received from other gameboy
	dec a													; did the other person cancel?
	jr nz, .doTrade											; if signal received wasn't $1, do the trade
...
.doTrade
	...
	ld [wTradedPlayerMonSpecies + 1], a		; to handle 2 bytes species IDs
	xor a
	...
	ld [wMonSpeciesTemp + 1], a				; to handle 2 bytes species IDs
	push bc
	call GetDexNumberBySpeciesID			; needed for next function
	callab CheckPokedexOwned				; check if received mon is a new dex entry
	ld a, 0
	jr nz, .setNewDexEntry
	inc a
.setNewDexEntry
	ld [wTradedMonIsNewDexEntry], a			; need to store this info for later
	ld hl, wEnemyMons
	pop bc
	ld a, c
	ld bc, wEnemyMon2 - wEnemyMon1
	...
	ld a, [hl]								; to handle 2 bytes species IDs
	ld [wTradedEnemyMonSpecies + 1], a		; to handle 2 bytes species IDs
	ld a, 10
