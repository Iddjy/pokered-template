...
.next
	...
	call EnableLCD
IF DEF(_RED)				; which Pokemon to show first on the title screen for RED version
	ld bc, CHARMANDER		; use bc instead of a since species ID are now 2 bytes
ENDC
IF DEF(_BLUE)				; which Pokemon to show first on the title screen for BLUE version
	ld bc, SQUIRTLE			; use bc instead of a since species ID are now 2 bytes
ENDC
...
; Keep looping until a mon different from the current one is picked.
	...
	ld b, [hl]					; to handle 2 bytes species IDs
