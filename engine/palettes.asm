...
SetPal_Battle:
	...
	ld a, [hld]
	or c
	jr nz, .playerMonsterPalette
	ld b, PAL_PLAYER					; palette for the player's backsprite
	ld a, [wBattleType]
	dec a
	jr nz, .enemyPalette
	ld b, PAL_OLDMAN					; palette for the Old Man's backsprite
	jr .enemyPalette
.playerMonsterPalette
...
SetPal_Pokedex:
	...
	ld bc, $10
	call CopyData
	callab DeterminePaletteIDWithinPokedex	; use a different list of palettes indexed by dex number instead of species ID
	ld a, d									; previous function returns result in d
	ld hl, wPalPacket + 3
	ld [hl], a
	ld hl, wPalPacket
...
; c=0: use monster species in wWholeScreenPaletteMonSpecies to determine palette
; c!=0: use palette ID in c
SetPal_PokemonWholeScreen:
	...
	and a
	jr nz, .next
	ld hl, wWholeScreenPaletteMonSpecies	; to handle 2 bytes species IDs
	call DeterminePaletteIDOutOfBattle
.next
	ld [wPalPacket + 1], a
	ld hl, wPalPacket
...
; add this to make the pokeballs red in the Link battle versus screen
SetPal_LinkStart:
	ld a, PAL_POKEBALL			; for reasons I don't understand, at the start of the battle, the OBP0 palette does not use
	push af						; the palette we set here, but one where the dark shade becomes the light shade and the light
	jr SetPal_LinkVersusText	; shade becomes white, while the black and white ones remain unchanged, so we can't use the
								; same palette for both the start and the end of a link battle (since at the end it doesn't do that)
...
SetPalFunctions:
	...
	dw SetPal_LinkStart				; add this to make the pokeballs red in the Link battle versus screen
	dw SetPal_LinkEnd				; add this to make the pokeballs red in the Link battle versus screen
...
; added this function to mutualize code
InitPalPacket:
...
DeterminePaletteID:
	bit TRANSFORMED, a				; if mon is transformed, use Ditto's palette
	ld a, PAL_PINKMON				; DITTO is pink not grey
	ret nz
DeterminePaletteIDOutOfBattle:
	ld a, [hli]						; to handle 2 bytes species IDs
	ld d, a							; to handle 2 bytes species IDs
	ld e, [hl]						; to handle 2 bytes species IDs
