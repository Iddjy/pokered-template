...
; Load a new map.
	...
	ld [wJoyIgnore], a
	call LoadMapData
	callab MaybeUpdateEncounterTable	; update encounter table in case we're entering a dark map while FLASH is already active
...
OverworldLoopLessDelay::
	...
	callab CheckIfPCBoxWasFilledUp		; warn the player if they filled up their PC box upon catching a mon.
										; Since this displays a textbox, it cannot be placed before the call to JoypadOverworld
										; because DisplayTextID clobbers wSpriteIndex which interferes with sprites which should
										; become hidden after a battle (eg Static mons), which happens inside EndTrainerBattle,
										; which is called by RunMapScript, which is called by JoypadOverworld...
	...
	or b										; to handle 2 bytes species IDs
	jp nz, .newBattle
	ld a, [wd730]
...
.battleOccurred
	...
	xor a
	ld [hJoyHeld], a
	ld [wEngagedTrainerClass], a			; reset this here so that it's called after every type of battle (except link battles, but since the only way to exit the colosseum is to reset the game, it doesn't matter)
	ld a, [wCurMap]
	cp CINNABAR_GYM
...
; this is for handling "outside" maps that can't have the 0xFF destination map
	...
	callab CheckDarkMaps		; check if new map requires FLASH
	jr .done
...
.goBackOutside
	...
	ld [wMapPalOffset], a
	ld hl, wd730
	res BIT_FLASH_ACTIVE, [hl]	; reset FLASH flag when stepping outside
.done
	ld hl, wd736
...
ExtraWarpCheck::
	...
	jr z, .useFunction1
	push de
	ld hl, Function2Array
	ld de, $0001
	call IsInArray			; use an array instead of listing them all one by one
	pop de
	jr c, .useFunction2
	ld a, [wCurMapTileset]
...
; add this to check for more maps without taking up too much space
Function2Array:
...
HandleFlyWarpOrDungeonWarp::
	...
	call LeaveMapAnim
	ld hl, wd732			; added this for secret passages
	bit 7, [hl]
	jr z, .skipLastMap		; if bit isn't set, then player didn't use a secret passage
	res 7, [hl]
	ld a, [wd0b5]
	cp $ff					; $ff means don't overwrite wLastMap
	jr z, .skipLastMap
	ld [wLastMap], a  		; update wLastMap here in case secret passage links 2 dungeons that aren't in the same town
.skipLastMap				; this way when exiting the target map, we don't find ourselves in glitch city, and palette is changed while screen is off
...
CheckForJumpingAndTilePairCollisions::
	...
	and a
	ld a, [wd736]
	bit 3, a										; use this bit to detect directional collisions
	res 3, a
	ld [wd736], a
	jr nz, CheckForTilePairCollisions.foundMatch    ; bit set means collision
	bit 6, a ; is the player jumping?
	ret nz
; if not jumping
...
.currentTileMatchesFirstInPair
	inc hl
	ld a, [hli]								; bugfix: increment hl otherwise subsequent checks are incorrect		
	cp c
	jr z, .foundMatch
	jr .tilePairCollisionLoop
...
TilePairCollisionsLand::
	db CAVERN, $20, $05
	db CAVERN, $41, $20	; replaced $05 with $20 to adapt to new tile used in some blocks
	db CAVERN, $41, $2A
	db FOREST, $30, $2E
	db CAVERN, $2A, $05
	db CAVERN, $05, $21
...
TilePairCollisionsWater::
	...
	db FOREST, $48, $2E
	db CAVERN, $14, $05
	db CAVERN, $14, $41	; added since it is now used by some blocks
	db $FF
...
; so the old value of c is used. UpdateSprites is always called before this function,
; and UpdateSprites always sets c to 0xF0. There is no 0xF0 background tile, so it
...
CollisionCheckOnWater::
	...
	ld a, [wSpriteStateData1 + 12] ; the player sprite's collision data (bit field) (set in the sprite movement code)
	and d ; check if a sprite is in the direction the player is trying to go
	jr nz, .collision					; fix bug described above
...
.finishUp
	...
	ld [H_LOADEDROMBANK], a
	ld [MBC1RomBank], a
	jpab CheckAlternateMusic	; to allow maps to have alternate music
