...
Char58:: ; prompt
	...
	jp z, .ok
	ld a, "▼"
	Coorda 18, 17			; display the blinking arrow in the textbox frame
.ok
	call ProtectedDelay3
	call ManualTextScroll
	ld a, "─"				; since the arrow is in the textbox frame now, when it blinks off, replace it with a piece of frame instead of a blank
	Coorda 18, 17			; display the blinking arrow in the textbox frame
Char57:: ; done
	pop hl
	ld de, Char58Text
...
Char49::	; page
	push de
	ld a, "▼"
	Coorda 18, 16			; leave it here so that it doesn't blink because now the blinking only works on menu border tiles, not dex pages
	call ProtectedDelay3
	call ManualTextScroll
	coord hl, 1, 10
	lb bc, 7, 18
...
.matchFound
	ld a, [hl]
	call PlaySoundWaitForCurrent	; wait for current sound to finish to avoid overlapping problems (can be seen with the NPC giving TM59 on Silph Co. 2F)
...
TextCommandSounds::
	...
	db $14, SFX_LEVEL_UP
	db $15, SFX_SS_ANNE_HORN
	db $16, SFX_HEALING_MACHINE
